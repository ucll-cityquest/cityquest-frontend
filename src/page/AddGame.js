...
class AddGame extends Component {
  state = {
    ...
    lng: 0,
    description: '',
    newQuestion: { question: '', lat: 0, lng: 0, answers: [], correctAnswer: -1, newAnswer: '' , comment: '' },
    questions: [],
  };
  ...
  handleAddAnswer = () => {
    this.setState({
      newQuestion: { ...this.state.newQuestion, answers: [ ...this.state.newQuestion.answers, this.state.newQuestion.newAnswer ], newAnswer: '' }
    });
  };
  ...
  handleChangeCorrectAnswer = event => {
    ...
    this.setState({
      newQuestion: { ...this.state.newQuestion, correctAnswer: (event.target.checked ? index : -1) },
    });
  };
  ...
  handleSubmit = async () => {
    ...
    await fetch(createApiUrl("games"), {
      ...
      body: JSON.stringify({ name: t.name, location: t.city, description: t.description, coordinates: { lat: Number(t.lat), lng: Number(t.lng) }, questions: t.questions.map(e => ({
        question: e.question,
        extraInfo: e.comment,
        ...
        <form className={classes.container} noValidate autoComplete="off">
          ...
          <div className={classes.addQuestion}>
            ...
            {this.state.newQuestion.answers.map((e, i) => (
              <div key={i}>
                <p className={classes.newQuestionAnswer}>{e}</p><Checkbox value={'i'+i} checked={i===this.state.newQuestion.correctAnswer} onChange={this.handleChangeCorrectAnswer} />
              </div>
            ))}
            <div>
              <TextField
                ...
                variant="outlined"
              />
              <Button variant="outlined" color="primary" className={classes.button} onClick={this.handleAddAnswer} >
                Add answer
              </Button>
            </div>
            <div>
            ...
            <div>
              <Button variant="outlined" color="primary" className={classes.button} onClick={this.handleAddQuestion} >
                  Add question
              </Button>
            </div>
          </div>
          {this.state.questions.map((e, i) => (
            <div className={classes.cardContainer} key={i}>
              <Card className={classes.card}>
                <CardContent>
                  <IconButton className={classes.button} aria-label="Delete" onClick={this.handleDeleteQuestion(i)}>
                    <DeleteIcon />
                  </IconButton>
                  ...
                  <div className={classes.answers}>
                    {e.answers.map((answer, index) => (
                      <Typography key={index} component="p" className={[ (index > 0 ? classes.answer : ''), (index === e.correctAnswer ? classes.correctAnswer : '') ].join(' ')}>
                        {answer}
                      </Typography>
                    ))}
                    <Typography className={classes.pos} color="textSecondary">
          ...
          <Button variant="outlined" color="primary" className={[classes.button, classes.submit].join(' ')} onClick={this.handleSubmit} >
              Save game
          </Button>
        </form>
      </React.Fragment>
